...
readr::write_csv(log_pig, paste0(path_out, "log_pig_summary_stat.csv"))
```
...
# then paste into .rprofile using below code, should NOT be in this directory
usethis::edit_r_profile()

# when running, seems like we have to source it, then set the options to it
source("~/.rprofile")
...
out <- ghcnd(stationid = "USC00082046", date_min = "2010-01-01") %>%
    filter(year >= 2015)
```
