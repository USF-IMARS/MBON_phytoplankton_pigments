...
Dataset includes metadata, pigment concentrations, and size class calculations
...
## 3.1 Plot Concentration of Pigments vs Time
...
## 3.2 Pigment Concentration vs Total Chlorophyll-a 
...
## 3.3 Plot Histogram and Violin Plot
...
## 3.5 Size Classes using Diagnostic Biomarker

Boxplots for each region and season of biomarkers for size classes
- micro (> 20um)
- nano (2 - 20um)
- pico (< 2um)
...
## 3.5.1 Size Classes for Each Station
...
# pig_dat %>%
#   mutate(
#       .by = name_of_water_body,
#     .keep = "used",
#     lat, lon, name_of_water_body,
#     station,
#     
#     rank = if_else(
#         str_detect(name_of_water_body, "(?i)keys"),
#         fct_reorder(station, -lon),
#         fct_reorder(station, lat)
#     ),
#     rank = as.numeric(rank)
#   ) %>%
# arrange(name_of_water_body, rank)
```
...
# save table as word docx (editable) or html (better formatted)
if (FALSE) {
  # as word .docx
  frac_gt %>%
    gtsave(
      filename = here(
        "data", "plots", "plot_pigment",
        glue(
          "fract_table",
          "{format(Sys.time(), '_%Y%m%d_%H%M%S')}.docx"
        )
      )
    )
...
if (FALSE) {
  ...
  save_plot(
    filename = here(
      "data", "plots", "plot_pigment",
      glue(
        "pi_ppc_chl_all_water_body",
        "{format(Sys.time(), '_%Y%m%d_%H%M%S')}.jpeg"
      )
    ),
    plot        = plt_pi_all_water_body,
    base_height = 6,
    base_width  = 10,
    dpi         = 600,
    units       = "in",
  ...
  save_plot(
    filename = here(
      "data", "plots", "plot_pigment",
      glue(
        "pi_box_ppc_chl_all_water_body",
        "{format(Sys.time(), '_%Y%m%d_%H%M%S')}.jpeg"
      )
    ),
    plot        = pi_box_seasonal_water_body,
    base_height = 6,
    base_width  = 10,
    dpi         = 600,
    units       = "in",
