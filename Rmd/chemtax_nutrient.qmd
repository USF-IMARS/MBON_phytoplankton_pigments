...
Load standard set of libraries and set global options.
...
cmtx_dat <-
  ...
  mutate(season = fct_relevel(season, szn)) %T>% 
  print() %>%
  mutate(
...
```{r plt-pft-env, fig.width=25, fig.asp=0.5}
cmtx_nutr %>%
  select(
...
nmds <- 
  ...
  # metaMDS(., distance = "bray") %T>%
  metaMDS(., distance = "jaccard") %T>%
  print()
# Call:
...
data_scores <-
  cmtx_dat %>%
  bind_cols(scores(nmds)$sites) %T>%
  print()


# filter for near-off pairs
set.seed(1355453)
nmds_filt <- 
  cmtx_dat %>%
...
data_scores_filt <-
  ...
  mutate(labels = case_when(
      ...
      str_detect(station, "12|18") ~ "Offshore"
  )) %T>%
  print()
```
```{r}
...
plt_nmds <-
  ...
  geom_point(
    size = 4, aes(color = season, shape = labels),
    alpha = 0.75
  ) +
  ...
  labs(
    shape = "Station",
    color = "Season",
    subtitle = paste0("Stress: ", round(nmds$stress, 2))
