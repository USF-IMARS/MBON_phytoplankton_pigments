...
env_cmtx <- 
    read_csv(here("data", "processed", "cmtx_env_data.csv")) %>%
    mutate(
...
# CHEMTAX files
cmtx_avgs <- 
    read_csv(here("data", "processed","chemtax_with_metadata.csv")) %>%
    mutate(
        season = fct_relevel(season, 
                             "Winter", "Spring", "Summer", "Autumn")
        )
...
```{r func-ord-plot}
ggPCoA2 <- function(data, pc = 12, level = 0.68) {
    
  dis  <- vegdist(data[, -1], method = "bray", na.rm = TRUE)
