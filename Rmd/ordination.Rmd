...
env_data_matrix <- env_cmtx %>% 
    ...
    na.omit() %>% 
    rename(Salinity= sal, Temp = temp, PAR= par)
    

env_pca <- prcomp(env_data_matrix[1:3], scale. = TRUE)
ggplot2::autoplot(env_pca, size = 5, data = env_data_matrix, colour = "season", loadings = T,
...
ggord(cmtx_cca, size = 4, grp_in = env_cmtx %>% drop_na(temp,par:sal) %$% as.factor(season),
      ellipse = F, ptslab = F, addsize = 6, ylim = c(-6.25, 6.25), xlim = c(-4, 5),
