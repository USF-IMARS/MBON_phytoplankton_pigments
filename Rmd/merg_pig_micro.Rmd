...
df_type_counts <-
    ...
    left_join(cmtx_avgs_raw, by = c("sample" = "sample_id", "func_type" = "taxa")) %>%
...
indices <- as.vector(distinct(conv_facts, indx))

rand_indx <- sample(indices$indx, 17, replace = FALSE)
...
cf_avgs <- 
    ...
    filter(indx %in% rand_indx , conc > 0 & micro_count > 0) %>%
...
cmtx_micro <- conv_facts %>% 
    ...
    pivot_wider(id_cols = c(indx), names_from= func_type, values_from = c(conc, micro_conc))

cmtx_micro_log <- conv_facts %>% 
    left_join(cf_avgs, by = "func_type") %>%
    # multiply conversion factors by microscopy counts to get T chla of counted functional type
    mutate(micro_conc = micro_count * avg_cf, log_mic_conc = log(micro_conc + 1), log_conc = log(conc + 1)) %>% 
    select(indx, func_type, log_mic_conc, log_conc) %>% 
    pivot_wider(id_cols = c(indx), names_from= func_type, values_from = c(log_conc, log_mic_conc))
...
# relation <- lm(conc_diat~micro_conc_diat, cmtx_micro)
# print(summary(relation))
# cmtx_micro <- conv_facts %>% 
#     left_join(cf_avgs, by = "func_type") %>%
#     # multiply conversion factors by microscopy counts to get T chla of counted functional type
#     mutate(micro_conc = micro_count * avg_cf) %>%
#     filter(micro_conc > 0 & conc > 0) %>% 
#     pivot_wider(id_cols = c(indx), names_from= func_type, values_from = c(conc, micro_conc)) %>% 
#     filter(indx != "17-3" & indx != "3-35" & indx != "4-39")


plot_micro_cmtx <- function(.data, .func_type) {
    ggplot(.data, aes_string(paste0("conc_",.func_type), paste0("micro_conc_",.func_type))) +
...
# ggplot(cmtx_micro, aes_string(x="conc_cyano", y="micro_conc_cyano")) +
#     blandr.draw()

blandr.draw(cmtx_micro$conc_cyano, cmtx_micro$micro_conc_cyano, plotTitle = "Cyanobacteria")
blandr.draw(cmtx_micro$conc_crypto, cmtx_micro$micro_conc_crypto, plotTitle = "Cryptophytes")
blandr.draw(cmtx_micro$conc_chloro, cmtx_micro$micro_conc_chloro, plotTitle = "Chlorophytes")
blandr.draw(cmtx_micro$conc_pras, cmtx_micro$micro_conc_pras, plotTitle = "Prasinophytes")
blandr.draw(cmtx_micro$conc_hapt, cmtx_micro$micro_conc_hapt, plotTitle = "Haptophytes")
blandr.draw(cmtx_micro$conc_dino, cmtx_micro$micro_conc_dino, plotTitle = "Dinoflagellates")
blandr.draw(cmtx_micro$conc_diat, cmtx_micro$micro_conc_diat, plotTitle = "Diatoms")

blandr.output.text(cmtx_micro$conc_cyano, cmtx_micro$micro_conc_cyano)

cyano_stat <- blandr.statistics(cmtx_micro_log$log_conc_din, cmtx_micro_log$log_mic_conc_dino)
# cyano_stat <- blandr.statistics(micro_conc_cyano ~ conc_cyano, data = cmtx_micro)
