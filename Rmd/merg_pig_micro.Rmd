...
A work in progress, they may need to be examined further
...
## 2.3 Read FWRI Microscopy
```{r microscopy}
df_micro <-
  # path to FWRI microscopy
  dir_ls(
    here("data", "raw"),
  ...
  mutate(
    station = str_remove(station_id, "^(Keys MBON |NOAA-AOML )"),
...
# 3.0 ---- Index to Merge HPLC with Microscopy ----

Based on station, date and depth ID (i.e. surface or bottom sample)
...
# 4.0 ---- Group HPLC Pigment and Microscopy ----
...
taxa_df <-
  ...
  mutate(
    ...
    genus    = str_split(taxa_fix, "_", 2, simplify = TRUE)[, 1],
    species  = str_split(taxa_fix, "_", 2, simplify = TRUE)[, 2]
  ) %>%
  ...
  filter(
    !str_detect(
      genus,
      str_c(
        "(?i)Copepods", "Detritus", "Zooplankton", "Foraminifera",
...
# 5.0 ---- Categorize Taxa into Functional Types ----
...
## 5.1 List of Genera and Taxa Names
...
df_class <-   
    ...
    unite("func_type", pft1, pft2, na.rm = TRUE, remove = FALSE) %>%
    
    drop_na(taxa_fix) %>% 
    mutate(taxa = str_to_lower(taxa),
           func_type = na_if(func_type, "")
           ) %>%
    relocate(func_type, .after = taxa_fix) %>% 
    # filter out most columns
    select(taxa,func_type)
  ...
  pft_bland %$%
    walk2(
      ...
      ggsave(
        filename =
          here(
            "data", "plots", Sys.Date(), "altman-bland",
            glue(
              "{y}",
