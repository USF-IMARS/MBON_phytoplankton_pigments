...
df_type_counts <- micro_longer %>% 
    group_by(indx) %>%
    summarize(diat = sum_func_type("diat", indx),
              chloro = sum_func_type("chloro", indx),
              crypto = sum_func_type("crypto", indx),
              dino = sum_func_type("dino", indx),
              cyano = sum_func_type("cyano", indx),
              prasin = sum_func_type("pras", indx),
              hapt = sum_func_type("hapt", indx),
              other = sum_func_type("other", indx)
    ) 

df_pig_filt2 <- df_pig_filt %>% 
    left_join(cmtx_avgs, by=c("sample"="sample_id")) %>%
