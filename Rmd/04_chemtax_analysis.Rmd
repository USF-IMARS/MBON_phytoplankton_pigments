...
NOTE: Moved Code across multiple files. Code here was commented out if we need
      to come back to it.
...
## 2.3 Merge CHEMTAX Results with Metadata
...
#   filter(!str_detect(station, "60|21.5|24|^3$|9B|9.5")) %T>% print()
# 
...
#   rowwise() %>%
#   mutate(
#     tot_chl = sum(c_across(chloro:pras)),
#     # across(chloro:pras, ~round(.x, 4))
#   ) %>%
#   ungroup() %>%
#   mutate(
#     # across(chloro:pras, list("percent" = ~ .x/tot_chl))
#     across(chloro:pras, ~ .x/tot_chl)
#   ) %>%
#   
#   pivot_longer(
#     cols      = chloro:pras,
...
#     )
#   ) %>%
#   distinct(cruise_id, date_time_utc, station, depth, .keep_all = TRUE) %T>% print()
# 
# # ---- save chemtax data with metadata
# if (
#   !file_exists(here(dir_data_save, "chemtax_w_meta_all_wide.csv")) |
...
### TODO: add station specific box plots
```{r}
# cmtx_l %>%
...
# cmtx_l %>%
#   filter(.by = c(station, names), n() > 3) %>%
#   mutate(.by = c(station, names), stn_label = glue("{station} (N = {n()})"), .after = station) %>%
#   distinct(stn_label)
```
...
# 5.0 ---- Perform Kruskal-Wallis and Dunn Test ----

## 5.1 K-W Test with PFT on season
...
#   mutate(p2 = scales::label_pvalue(0.05)(p))
# 
...
#     ) %>%
#   mutate(p2 = scales::label_pvalue(0.05)(p.adj))
# 
# # Station Dunn's test
# cmtx_dunn_stn <-
#   cmtx_l %>%
...
#   plt            = last_plot(), # edit if want specific plot
#   save_location  = dir_plt_save,
#   save_name      = "pft_season_avg_prcnt",
#   verbose        = TRUE,
#   overwrite      = FALSE,
#   # overwrite      = TRUE,
#   time_stamp_fmt = "%Y%m%d_%H%M%S",
...
# 7.0 ---- Time Series of 3 Stations with 2 PFTs -----

## 7.1 Plot: Calculate Fraction and Plot 3 Stations with 2 PFTs (Diatom and Cyano)
...
# # ---- save plot
# save_gg(
#   plt            = cmtx_sub_ln_plt, # edit if want specific plot
#   save_location  = dir_plt_save,
#   save_name      = "stn_subset_frac_diat_cyano",
#   verbose        = TRUE,
#   overwrite      = FALSE,
#   time_stamp_fmt = "%Y%m%d_%H%M%S",
#   device         = "jpeg",
