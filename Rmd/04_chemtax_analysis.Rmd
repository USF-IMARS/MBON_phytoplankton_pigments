...
box_lg <-
  cmtx_l %>%
  filter(phyto %in% c("cyano2", "cyano4", "chloro", "hapt6")) %>%
  mutate(
    .by = c(station, names),
    stn_label = glue("{station} (N = {n()})"),
    .after = station
...
box_lg_stn <-
  ...
  facet_grid(facet = stn_label ~ names) +
...
walk2(
  ...
  .y = list(box_lg, box_sm, box_lg_stn, box_sm_stn, box_lg_stcked, box_sm_stcked),
  \(.x, .y, ...) {
    .x %>%
      str_to_lower() %>%
      str_replace_all("\\s", "_") %>%
...
cmtx_fract_l %>%
  ...
  arrange(station) %>%
  summarise(
    .by = c(crusise_id, station, season, names, phyto),
    across(
      .cols  = frac,
      .fns   = (\(x) mean(x, na.rm = TRUE)),
      .names = "{.col}_avg"
