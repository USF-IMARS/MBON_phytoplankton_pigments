...
# gt_pft <-
 cmtx_avg_fract_l %>%
     pivot_wider(
       data         = .,
       id_cols      = c(names, phyto), # *optional* vector of unaffected columns,
       names_from   = c(season), # category column(s) to pivot wide
       values_from  = c(percent_avg), # value column(s) that hold data for each category column
       names_sep    = "_"
       # names_repair = janitor::make_clean_names
       ) %>%
  ...
  # cols_hide(c(phyto, frac_avg)) %>%
  cols_hide(phyto) %>%
  # cols_label(percent_avg = "Average (%)") %>%
  tab_options(
