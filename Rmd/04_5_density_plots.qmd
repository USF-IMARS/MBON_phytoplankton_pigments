...
plt_cmtx_den_hist <-
  cmtx_l %>%
  # convert to log scale for concentrations
  ...
  mutate(
    ...
    hist_per = map(
      ...
      .y = phyto,
      ~ .x %>%
        ggplot(aes(percent, fill = season)) +
        geom_histogram(
          position = "stack",
          bins = 40,
          na.rm = TRUE
    ...
    dens_per = map2(
      ...
      .y = phyto,
      ~ .x %>%
        ggplot(aes(percent, y = season, fill = season)) +
        ggridges::geom_density_ridges(
          na.rm = TRUE,
          alpha = 0.25
        ) +
...
## 3.2 Hist/Density: Concentration per Season

```{r plotting-chemtax-conc}
plt_cmtx_den_hist$dens
plt_cmtx_den_hist$hist
```

## 3.3 Hist/Density: Percent per Season

```{r plotting-chemtax-percent}
plt_cmtx_den_hist$dens_per
plt_cmtx_den_hist$hist_per
```