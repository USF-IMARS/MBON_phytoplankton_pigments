...
plot.pig1 <- function(.var, .title) {
    list(
        ...
        theme_bw(),
        labs(
            title = paste("Average Concentration per Season of", .title),
            x = "Season",
            y = expression(paste("Concentration (mg  ", m ^ -3, ")"))
        )
    )
...
# TODO: either load pig_dat here or higher
...
cmtx_fract <- cmtx_avgs %>% 
    ...
    #        hapt = hapt6 + hapt8,
    #        total = 
    #            chloro + crypto + diat + dino + hapt 
    #        + pras + cyano) %>% 
    mutate(total = chloro + crypto + cyano2 + cyano4 + diat1 +
               diat2 + dino + hapt6 + hapt8 + pras,
           year = year,
           season = season,
           station = station,
...
cmtx_fract %>% 
    mutate(frac_diat = frac_diat1 + frac_diat2,
           frac_cyano = frac_cyano2 + frac_cyano4) %>% 
    pivot_longer(frac_chloro:frac_cyano, names_to = "func_type", values_to = "frac", names_prefix = "frac_") %>% 
...
sec <- help_secondary(data = cmtx_fract2,
                      primary = c(season, avg_fract),
                      secondary = c(season, tot_chl_a_avg),
                      # method = c("range", "max", "fit", "ccf", "sortfit"))
                      method = "max",
                      name = "Average Total Chl-a")

cmtx_fract_graph <- cmtx_fract2 %>% 
    ...
    labs(x = "Season", y= "Proportion to Total Chl-a",
...
cowplot::save_plot(filename = paste0(root,"/data/plots/phyto_func_type_bar_ratio_season", 
           format(Sys.time(), '_%Y%m%d_%H%M%S'),
           ".jpg"), 
          plot = last_plot(),
          base_height = 14,
