...
## 2.2 Line Plots: Average Per Season per PFT
```{r line-plt}
cmtx_all <- cmtx_all %>%
    select(-rms) %>%
    pivot_longer(
...
cmtx_avgs <-
    full_join(meta_data, cmtx_avgs_raw, by = join_by("sample" == "sample_id")) 
...
## 3.2 Box Plot: Individuals Box Plots for Each PFT per Season
...
# 4.0 ---- PFT multi-plots ----
## 4.1 PFTs with a more dynamic range of values
...
## 4.2 PFTs with a more dynamic range of values
...
sv_arg <-
  ...
  c(
    .,
    list(
      sm_exist = dir_ls(sv_arg$sv_loc, regexp = "boxplot.*sm") %>% is_empty(),
      lg_exist = dir_ls(sv_arg$sv_loc, regexp = "boxplot.*lg") %>% is_empty()
    )
  )
