...
# 2.0 ---- Average Plot (1 Value per PFT per Season) ----
## 2.1 Load Average Phytoplankton Concentrations from CHEMTAX 
This takes each season and averages them together.
...
## 2.2 Plot: Average Per Season per PFT (Line)
Not super useful
```{r line-plt}
cmtx_all <- cmtx_all %>%
    select(-rms) %>%
...
```
# 3.0 ---- Box Plots ----
## 3.1 Load All Phytoplankton Concentrations
```{r load-comb-data}
cmtx_avgs_raw <- 
...
if (!file_exists(here("data", "processed", "chemtax_w_meta.csv")) |
    !file_exists(here("data", "processed", "chemtax_w_meta_rm_dup.csv"))) {
...
## 3.2 Plot: Individuals for Each PFT per Season (Box)
...
# 4.0 ---- PFT multi-plots ----
## 4.1 Plot: FTs with a more dynamic range of values (Box)
...
## 4.2 Plot: PFTs with Greater Dynamic Range (Box)
...
## 4.3 Save Large and Small Range Boxplots
```{r save-plt}
sv_arg <-
  list(
    sv_loc    = here("data", "plots", "boxplots"),
    sv_nm     = str_c("cmtx_boxplot_", c("sm", "lg")),
    overwrite = FALSE
  ) %>%
  c(
    .,
    sm_exist = dir_ls(.$sv_loc, regexp = .$sn_nm[1]) %>% is_empty(),
    lg_exist = dir_ls(.$sv_loc, regexp = .$sn_nm[2]) %>% is_empty()
  )
...
if (sv_arg$lg_exist | sv_arg$overwrite) {
  save_plot(
    ...
    cli::cli_alert_info(c("Skipping saving of {.file cmtx_boxplot_lg} ",
                          "because exists ",
...
if (sv_arg$sm_exist | sv_arg$overwrite) {
  save_plot(
    ...
    cli::cli_alert_info(c("Skipping saving of {.file cmtx_boxplot_sm} ",
                          "because exists ",
                          "and/or overwrite is set {.var FALSE}"))
}
rm(sv_arg)
...
```
## 6.1  Stations 10, 16, WS: Plot Cyano and Diat vs time and 
```{r cmtx-summary}
# cmtx_season_avgs <-
cmtx_avgs_w %>%
  group_by(season) %>%
...
## 6.2 Plot: Calculate Fraction and Plot 3 Stations with 2 PFTs (Diatom and Cyano)
...
cmtx_sub_ln_plt
```
## 6.3 Save Subset of PFT and Stations
```{r save-dia-cyano}
sv_arg <-
  list(
...
# 7.0 ---- Average of Season and Stations per PFT ----
```{r view-avgs}
# by season
cmtx_fract %>%
