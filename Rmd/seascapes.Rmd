...
```{r load-chm-ctd}
dir_plt_save <- here("data", "plots", "seascapes")
...
# ---- CHEMTAX with CTD
env_cmtx <-
  here("data", "processed", "ordination") %>%
  dir_ls(regexp = "cmtx_env_data") %>%
  last_mod() %>%
  read_csv(show_col_types = FALSE) %>%
  mutate(
    season = fct(season, seasons),
    yr_mth = format(date_time_utc, "%Y.%m")
  ) %>%
  relocate(yr_mth, .after = 2) %>%
  rename("lon" = lon.x, "lat" = lat.x) %T>% print()
```
...
cmtx_sea <-
    ...
    unnest(sc) %>%
    select(-data) %>% 
    drop_na(seascape, temp, par:sal) %T>% print()

```

# CAP with Bray-Curtis?
```{r sc-cap}
seascspec <- select(cmtx_sea, chloro:pras)
...
for (szn in seasons) {
  ...
  save_gg(
    plt            = cca_szn,
    save_location  = dir_plt_save,
    save_name      = glue("seascape_cca_{str_to_lower(szn)}"),
    verbose        = TRUE,
    overwrite      = FALSE,
    time_stamp_fmt = "%Y%m%d_%H%M%S",
    device         = "jpeg",
...
set.seed(1355453)
nmds <- 
  metaMDS(seascspec, distance = "bray") %T>%
  print()
# Call:
...
plt_nmds <-
  ...
  geom_point(
    size = 4, aes(shape = season, color = factor(seascape)),
    alpha = 0.75
  ) +
...
Test significant difference between two or more groups of sampling units

```{r anonism}
# similarity on seascape
ano1 <-
  anosim(
    seascspec,
    cmtx_sea$seascape,
...
# Determine PFT Dominance by Season and Seascape

Which PFT dominates each seascape class by season?
