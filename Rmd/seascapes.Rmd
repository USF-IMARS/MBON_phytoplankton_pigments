...
unzip(here(seasc_path, "fknms_global_monthly.zip"), exdir = seasc_path)

# ---- load chemtax data ----
# dir2 <- "//data//processed//"
# path_out <-  paste0(root,dir2)
cmtx_avgs <- read.csv(here(path_out, "chemtax_w_meta_rm_dup.csv"))
env_cmtx <- read.csv(here(path_out, "cmtx_env_data.csv")) %>% 
    mutate(season= forcats::fct_relevel(season, "Winter", "Spring", "Summer", "Autumn"),
           date_time_utc = janitor::excel_numeric_to_date(date_time_utc)) 
...
file.seascapes <-
  fs::dir_ls(path = seasc_path,
             ...
             regexp = "^[^~]*\\.tif$") %>% 
    tibble(file = .) %>%
    mutate(yr_mth = stringr::str_remove(file, "\\.15\\.tif$"),
        yr_mth = str_extract(yr_mth, "[0-9]{4}\\.[0-9]{2}$")
           )
    # as_tibble() %>% 
...
seascapes_match <- left_join(mth_yr_df,
    ...
    na.omit() %>%
    arrange(yr_mth)
file.sea <- seascapes_match$yr_mth %>% 
    str_sort()
...
for (i in seq(file.sea)) {
    ...
    seascapes_temp <- extract_seascapes(temp_dat$file, cmtx_sea) %>% 
        mutate(yr_mth = temp_dat$yr_mth)
