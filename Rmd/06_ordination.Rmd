...
#' @param .group_var_col A grouping variable to add color to plot
#' @param .group_var_shape A grouping variable to add shapes to plot
...
gg_pcoa2 <- function(
  ...
  pcoa <-
    vegdist(
      # data[, -1],
      select(data, -contains(c(.group_var_col, .group_var_shape))),
      method = dist_meth,
      na.rm  = TRUE
    ) %>%
    # run PCoA
  ...
  dat_pcoa <-
    ...
    bind_cols("group" = data[[.group_var_col]], .) %>%
    rename_with(
      .cols = contains("Axis"),
  ...
  if (plt_vector) {
    ...
    vector_dat   <- select(vector_dat, -{{ .group_var_col }})
...
```{r plt-pcoa-pca, fig.width=15, fig.height=10}
# ---- PCoA on PFT groups
## calc and plot
pcoa_dat <-
  # select(cmtx_dat, season, chloro:pras) %>% # using all data
   select(env_cmtx, 
          station, 
          # season, 
          chloro:pras) %>% # using filtered data with CTD
  filter(str_detect(station, "10|12|18|16")) %>%
  gg_pcoa2(
    pc         = c(1, 2),
    .correct   = "none",
    # .group_var_col = "season",
    .group_var_col = "station",
    plt_vector = TRUE
  )
...
anova(manova_ctd_season)
plot(manova_ctd_season)
boxplot(manova_ctd_season)
    ...
    pairwise_p =
      map(
        ...
        .f =
          ~ szn_adonis2(
            # .data    = cmtx_dat,
            .data    = select(env_cmtx_filt, season, chloro:pras),
            pairs    = .x,
            pair_col = season,
            formula  = "season",
            strata   = NULL
...
# PFT vs year
adonis2(
  select(env_cmtx_filt, chloro:pras) ~ year,
  data   = select(env_cmtx_filt, year),
  method = "bray"
)
...
permanova_r_year <- 
  ...
  t() %>%
  as_tibble() %>%
  set_colnames(c("year1", "year2")) %>%
  unite(col = "year_pairs", year1, year2, sep = "|") %>%
  mutate(
    pairwise_p =
      map(
        .x = year_pairs,
        .f =
          ~ szn_adonis2(
            # .data    = cmtx_dat,
            .data    = select(env_cmtx_filt, year, chloro:pras),
            pairs    = .x,
            pair_col = year,
            formula  = "year",
            strata   = NULL
...
unnest(permanova_r_year, results)
permanova_r_year$results
