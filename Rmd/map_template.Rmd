...
#map of stations 10, 16, WS
exnt3 <- c(xmin = -81.9, # West
          xmax = -80.78, # East
          ymin = 24.4,# South
          ymax = 24.85   # North 
        )
ws_10_16 <- lat_lon_dat %>% 
    filter(station %in% c("WS", "10", "16"))
small.stn.map <- ggplot() +
  geom_sf(data = world2) +
  # geom_point(data = pig_dat,
    geom_point(data = ws_10_16,
             aes(
               x = lon,
               y = lat
               ##shape = station,
  ...
  coord_sf(xlim = exnt3[1:2], ylim = exnt3[3:4]) +
  labs(
    x = "",
    y = "",
    shape = "Station",
    fill = "Station") +
  theme_bw() +
  theme(text=element_text(family="serif", size=50),
         panel.grid.major = element_blank(), 
         panel.grid.minor = element_blank()
         )
