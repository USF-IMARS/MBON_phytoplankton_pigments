...
all_cruise <- 
    ed_search_adv(query = "Walton Smith",
...
#     temp <- try(ctd_downloads(cruiseID[i], loc, verbose = FALSE), silent = TRUE)
#     
#     }
tictoc::tic()
...
file_ctd <-
    ...
    mutate(
        # fix cruise IDs
        cruise_id = str_extract(basename(file), "\\w{2,3}\\d{4,5}"),
        cruise_id = case_when(
        ...
        station   = str_extract(file, "(?i)(sta|stn)_*(.*)\\.csv$", 2),
...
filter(file_ctd, str_detect(cruise_id, "WS2027"))
    ...
    #                 str_remove(., "(?i).*(stn|sta)") %>% # rm sta/stn
...
# ctd_dat_summary <- 
#     mutate(ctd_dat_summary,
#        cruise_id = if_else(str_detect(cruise_id, "WS20279"), "WS20278", cruise_id))

# unique(ctd_dat_summary$cruise_id)[str_detect(unique(ctd_dat_summary$cruise_id), "WS2027")]
    
# ctd_dat_summary <- here("data", "processed") %>%
#     dir_ls(regexp = "2023-04-07") %>%
#     read_csv(show_col_types = FALSE)

# unique(cmtx_avgs$cruise_id)[!unique(cmtx_avgs$cruise_id) %in% unique(ctd_dat_summary$cruise_id)]
# unique(ctd_dat_summary$cruise_id)[!unique(ctd_dat_summary$cruise_id) %in% unique(cmtx_avgs$cruise_id)]
...
if (FALSE) {
    ...
    filter(
        time < max(cmtx_avgs$date_time_utc) + period("1 month")
        & time > min(cmtx_avgs$date_time_utc) - period("1 month")
        ) 
    ...
    test <- env_cmtx %>%
    select(-c(pi, filter_type:pras, indicate_if_filters_are_replicates:bottle)) %>%
