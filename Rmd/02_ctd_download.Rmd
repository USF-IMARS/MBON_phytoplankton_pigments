...
dir_data_dwnld_save <- here("D:/", "MBON_phytoplankton_pigments", "data", "raw", "ctd")       # download path
...
file_ctd <-
  ...
  str_subset("ctd_all", negate = TRUE) %>%
  str_sort() %>%
  tibble(file = .) %>%
  mutate(
    base = basename(file),
...
## 4.1.2 Averages by Serial Processing
...
## 5.1 Save: Above 5 meter Averaged CTD Data
...
ctd_dat_summary %>%
  ...
  save_csv(
    .data          = .,
    save_location  = dir_data_avg_save,
    save_name      = "ctd_all",
    overwrite      = FALSE,
    verbose        = TRUE,
    time_stamp_fmt = NULL
...
## 6.1 Add Seasonal Factor and Water Body Name

This is for plotting of data only.

This is not saved with CTD data because when merged with either pigment data or
CHEMTAX data, these columns will already exist. 


-- NOTE: some station names are weird and correcting them was attempted, but some
         don't make it easy and are filtered out


---
Seasonal groups by month:
...
# merge water body and season
ctd_dat_szn_water <-
  ctd_dat_summary %>%
  mutate(
    # edit station names extracted from file names
    station = str_replace(station, "(57)(1|2|3)", "\\1.\\2"),
    station = str_replace(station, "WS2", "WS"),
    station = str_replace(station, "AMI1.2", "AMI1"),
    station = str_replace(station, "AM1", "AMI"),
    station = str_replace(station, "GP5.2", "GP5"),
    station = str_replace(station, "GP1.5", "GP1"),
    station = str_replace(station, "MRV2", "MR"),
    station = str_remove(station, "(_?A|B|_REDO|SURF|_RECAST|_2|UP)$"),
    
    water_body = case_match(
      station,
    ...
    water_body =
      case_when(
        str_detect(station, "(?i)Z0(3|4)_") ~ names(.env$water_body)[[3]],
        str_detect(station, "(?i)(EK|UK)_") ~ names(.env$water_body)[[1]],
        .default = water_body
      ),
    season = case_match(
    ...
    season = fct(season, names(szn)),
    do = if_else(do < 0, NA, do),
    .after = station
  ) %>%
  filter(
    !(is.na(season) |
...
## 6.2 Function: CTD Data Plot Function Through Time

```{r plt-ctd-data, fig.height=15, fig.width=20}
##%######################################################%##
#                                                          #
####                      CTD Plot                      ####
#                                                          #
...
#' CTD Plot
#'
#' Plot CTD environmental variables through time. X axis should be time and 
#' y axis should an environmental parameter. There is potential for 2 grouping
#' factors with one for color and one for faceting. If `.grp2` is left blank, 
#' it will use the first grouping as a facet.
#'
#' @param .dat Data.frame or tibble object
#' @param .var_y Y axis variable
#' @param .grp1 Grouping factor for color
#' @param .grp2 Grouping factor for facetting, default = `.grp1`
#' @param print_plt To print the resulting plot.
#' @param .var_x Optional: change x axis from a date time, but if time is not a
#'               column in the `.dat`, it will error.
#'
#'
#' @author Sebastian Di Geronimo (October 03, 2023)
#'
...
env_plt <-
  function(
    ...
    if (str_detect(.var_x, "time")) x_lab <- "Date"

    if (str_detect(.var_y, "(?i)par")) x_lab <- "Hour"
    ...
    plt <-
      ...
      geom_hline(
        data = select(.dat, -season),
        aes(
          yintercept = mean(.data[[.var_y]], na.rm = TRUE)
        ),
        color = "red",
        alpha = 0.5
      ) +
      geom_point(
        data  =  select(.dat, -all_of(.grp2)),
        color = "grey70",
        alpha = 0.5
      ) +
      geom_smooth(
      ...
      facet_grid(rows = .data[[.grp2]] ~ .) +
      guides(color = guide_legend(nrow = 1)) +
      theme(legend.position = "top")

    # add 1 year breaks if date time
    if (str_detect(.var_x, "time")) {
      plt <- 
        plt + 
        scale_x_datetime(
          date_breaks = "1 year",
    ...
    # print plot
    if (print_plt) suppressWarnings(print(plt))
  }
```


## 6.2.1 Plot: Envrionmental Variables Through Time, By Season and Region

Variables used:
- temp = temperature (degrees C)
- sal = salinity (unitless)
- do = dissolved oxygen (mg L^-1 ?)
- par = photosynthetically active radiation (? W per something)
- chlorophyll_concentration = ()
- CDOM = not sure what this is measuring ()
...
```{r plt-ctd-data, fig.height=15, fig.width=20}
# names of vars that can be used
names(ctd_dat_szn_water)

# used vars
env_var <-
  c(
    "temp", "sal", "do", "par", "chlorophyll_concentration", "CDOM",
    "chlorophyll_fluorescence", "beam_attenuation", "beam_transmission",
    "sea_water_sigma_t"
  )

# loop through vars and plot
for (i in env_var) {
  temp <- ctd_dat_szn_water

  # filter CDOM because didn't have much prior to 2021
  if (str_detect(i, "(?i)cdom")) {
    temp <- filter(temp, year(time) > 2020)
  }

  # PAR is better plotted hourly because it changes most in a day than time
  if (str_detect(i, "(?i)par")) {
    temp %>%
      mutate(hour = hour(time)) %>%
      filter(!if_any(all_of(i), \(x) is.na(x))) %T>%
      env_plt(
        .,
        .var_y = i,
        .grp1  = "season",
...
# temp %>%
#   filter(!if_any(all_of(env_var[1]), \(x) is.na(x))) %T>%
#   env_plt(
#     .,
#     .var_y = env_var[1],
#     .var_x = env_var[3],
...
## 6.2B Plot Added Names: Envrionmental Variables Through Time, By Season and Region

This is the same as 6.2A, but allows parallel looping to include proper y axis 
and title names using a list `env_var` with *var* for the column name and *name*
as the proper name.
...
if (FALSE) {
  ...
  cmtx_avgs <-
    here("data", "processed", "chemtax_analsysis", "chemtax_w_meta_wide.csv") %>%
    read_csv(show_col_types = FALSE) %>%
    mutate(
      cruise_id = str_extract(sample, "[A-Z]{2,3}[0-9]{4,5}"),
