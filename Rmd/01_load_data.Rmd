...
df_all <-
  ...
  mutate(
    across(tot_chl_a:tchl_a_tpg, \(x) if_else(!is.na(x), x, 0)),
    cruise_id = str_extract(sample, "\\w{1,3}\\d{4,6}")
    ) %>%
  relocate(cruise_id, .before = sample) %T>% print()
```
...
df_all <-
  ...
  mutate(
    ...
    station = 
      if_else(
        str_detect(station, "9B") & str_detect(cruise_id, "WS19210"), 
        "9.5", 
        station
        )
  ) 
...
dir_data_save <- here("data", "processed", "load_data")
...
# 7.0 Table: Latitudes and Longitude for Florida Keys
...
if (
  menu(
    c("Yes, make lat/lon table", "No, I'm okay!"),
    title = "Are you sure you want to proceed?"
  ) == 1) {
    
  shelf(gt)

  # create able for lat/lon
  lat_lon_dat <-
    df_all %>%
    filter(name_of_water_body == "Florida Keys") %>%
  ...
  if (FALSE) {
    # save location
    dir_plt_save <- here("data", "plots", "load_data")

    dir.create(dir_plt_save)

    # save table
    file_expr(
      loc            = dir_plt_save,
      file_base      = "fl_keys_stn_lat_lon",
      exts           = c("html", "docx"),
