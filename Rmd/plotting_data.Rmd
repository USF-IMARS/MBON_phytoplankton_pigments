...
#pigment concentration vs total chlorophyll a 
...
#histograms to look at pigment distributions
pig_long <- pig_dat %>%
    pivot_longer(cols = tot_chl_a:tchl_a_tpg, names_to = "pigment",
                 values_to = "value") %>%
...
ggplot(aes(fill = func_type, color = func_type, y=conc, x=station)) +
    ...
    facet_grid(rows = vars(year), cols = vars(season)) +
    labs(x = "Station", y= "Concentration of Chlorophyll-a", fill = "Phytoplankton Functional Type",
         color = "Phytoplankton Functional Type")
