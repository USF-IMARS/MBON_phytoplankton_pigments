...
librarian::shelf(
    ...
    # additional
    huxtable, gt, cowplot, ggthemes, vegan
)
...
pig_dat <-
    ...
    mutate(
        season = fct_relevel(season, levels = c("W", "Sp", "Su", "F")),
...
pig_long$plt[[5]]
 ...
 # pig_hist <- 
 #     lapply(unique(pig_long$pigment), function(i) {
 #  
 #         ggplot(pig_long[pig_long$pigment == i, ], 
 #                aes(value, fill = season)) +
 #             geom_histogram(position = "dodge") +
 #             labs(title = i) +
 #             scale_x_continuous(trans = ~ log10(. + 1e-6))
 #         })
...
#chemtax histograms
# TODO: fix this, the var should be available, but its not here
cmtx_longer2 %>% 
...
pig_dat %>% 
    ...
    pivot_longer(cols = f_micro:f_pico,
...
save_plot(
    filename = here("data", "plots", glue("phyto_func_type_bar", 
           format(Sys.time(), '_%Y%m%d_%H%M%S'),
           ".jpeg")), 
    plot = last_plot(),
    base_height = 8,
