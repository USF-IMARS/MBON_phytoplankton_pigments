...
pig_dat <-
    ...
    mutate(
        ...
        name_of_water_body = case_when(
            str_detect(station, "WS") 
            & str_detect(name_of_water_body, "Florida Bay") ~ "Florida Keys",
            str_detect(station, "60") 
            & str_detect(name_of_water_body, "Florida Keys") ~ "Florida Bay",
            .default = name_of_water_body
            )
        ) %>%
...
ggplot(pig_dat, 
    ...
    facet_grid(name_of_water_body ~ season) +
    theme_bw()
```
...
```{r chemtax-stacked-bar, fig.width=30, fig.height=15}
...
cmtx_long  %>% 
    ...
    theme_calc()
```
```{r chemtax-stacked-bar-year-season, fig.width=30, fig.height=15}
...
```
```{r chemtax-stacked-bar-conc-year-season, fig.width=30, fig.height=15}
