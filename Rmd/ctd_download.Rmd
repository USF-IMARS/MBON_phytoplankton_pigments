...
for (i in seq(cruiseID)) {
  try(ctd_downloads(cruiseID[i], loc, FALSE), silent = TRUE)
}
```
...
file.ctd <- 
    ...
    mutate(
        ...
        stations = basename(file)  %>% # extract file name
                   tools::file_path_sans_ext() %>% # remove extension
                   str_remove(., ".*((?i)stn|(?i)sta)") %>% # remove sta/stn
...
env_cmtx <- cmtx_avgs %>% 
    ...
    left_join(ctd_dat_summary, by = c("cruise_code" = "cruise_id", "station" = "stations")) %>% 
    mutate_all(~ifelse(is.nan(.), NA, .)) 


write.csv(env_cmtx, 
          file = here(loc, "..", "processed","cmtx_env_data.csv"))}
```
