...
#' @param grp_name The column name used to group the data.
...
map_data <- function(
  ...
  cli_alert_info("Working on: {conc_name} for {grp_name}")

  if (mean(.data[[col_name]], na.rm = TRUE) == 0) {
    cli_alert_warning(c(
      "Skipping: Average {conc_name} for ",
      "{grp_name} equals 0."
    ))
  ...
  loc_grid <-
    list(
      lon = seq(
        min(.data$lon, na.rm = TRUE) - adj,
        max(.data$lon, na.rm = TRUE) + adj,
        resolution
      ),
      lat = seq(
        min(.data$lat, na.rm = TRUE) - adj,
  ...
  if (rlang::is_null(base_map)) {
    cli_inform("\n")
    cli_alert_warning(c(
      "{.var base_map} is set to {.var NULL}. ",
      "Skipping plotting."
    ))
    return(dat_sp)
  }
